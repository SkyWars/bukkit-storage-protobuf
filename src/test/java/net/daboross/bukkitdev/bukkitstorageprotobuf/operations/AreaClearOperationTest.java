...
public class AreaClearOperationTest {
        ...
        for (int x = 0; x < lengthX; x++) {
            for (int y = 0; y < lengthY; y++) {
                for (int z = 0; z < lengthZ; z++) {
                    if (!operation.blocksCleared[x][y][z]) {
    ...
    private class TestOperation extends AreaClearOperation {

        private boolean[][][] blocksCleared;

        public TestOperation(int operationSize, int lengthX, int lengthY, int lengthZ) {
            super(operationSize, null, -20, -20, -20, lengthX, lengthY, lengthZ);
            blocksCleared = new boolean[lengthX][lengthY][lengthZ];
        }

        @Override
        protected void clearBlock(final int x, final int y, final int z) {
            if (blocksCleared[x][y][z]) {
                Assert.fail(String.format("Block double cleared at [%d][%d][%d]!", x, y, z));
            }
            blocksCleared[x][y][z] = true;
        }
    }
}
